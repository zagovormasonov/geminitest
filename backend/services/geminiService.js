    ...
    try {
      ...
      for (const modelName of models) {
        try {
          ...
          console.warn(`⚠️ Модель ${modelName} недоступна:`, modelError.message);
          
          // Проверяем, является ли это ошибкой регионального ограничения
          if (modelError.message.includes('User location is not supported')) {
      ...
      if (error.message.includes('API ключ')) {
        throw new Error('API ключ не настроен. Пожалуйста, добавьте GEMINI_API_KEY в переменные окружения Railway.');
      }
      
      if (error.message.includes('модели')) {
